	...
	*/
	...
	List<T> levels() const {
		...
		while (!toProcess.empty()) {
			...
			if (visit->_left)
				toProcess.push(visit->_left);
			if (visit->_right)
				toProcess.push(visit->_right);
		}
...
protected:
	...
	static unsigned int numNodesAux(Node *ra) {
		if (ra == NULL)
			return 0;
		return 1 + numNodesAux(ra->_left) + numNodesAux(ra->_right);
	}
	...
	static unsigned int depthAux(Node *ra) {
		...
		int leftDepth = depthAux(ra->_left);
		int rightDepth = depthAux(ra->_right);
	...
	static unsigned int numLeavesAux(Node *ra) {
		...
		if ((ra->_left == NULL) && (ra->_right == NULL))
			return 1;

		return numLeavesAux(ra->_left) + numLeavesAux(ra->_right);
	}
